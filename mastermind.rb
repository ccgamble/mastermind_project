module Mastermind
  ...
  def guess_eval(guess)
    elements = guess_elements_eval(guess)
    position = guess_position_eval(guess)
    success(guess, elements, position)
  end

  def guess_position_eval(guess)
    position = 0
  ...
  def elapsed_time(start, finish)
    time = (finish - start).divmod(60)
    "#{time.first} minutes, #{(time.last).round} seconds."
  end
